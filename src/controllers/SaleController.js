...
class SaleController{
    ...
    static async getAllSales(req, res) {
        try {
            ...
            res.status(200).json({ message: req.t('message.getSales'), data: result });
        } catch (error) {
            // Utilisation de la traduction pour "errorSale"
            res.status(500).json({ message: req.t('message.errorSale'), error: error.message });
        }
    }
    ...
    static async createSale(req, res, next) {
        ...
        try {
            ...
            res.status(error.statusCode || 500).json({ message: req.t('message.errorSale'), error: error.message });            
        }
        next()
    }
    ...
    static async deleteSale(req, res, next) {
        ...
        try {
            ...
            res.status(400).json({ message: req.t('message.errorSale'), error: error.message });
        }
        next()
    }
}
